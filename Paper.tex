...
\markright{Joey McDonald, Steve Delong}
...
\caption{Performance of tested online classification algorithms using different values of $p$.  The adaptive $p$ algorithm performs relatively well compared to the original algorithm ($p = 1$), and requires less than half as many negative updates. The $p = 0.1$ approach has ver poor recall, but requires far fewer updates.  The majority vote classifier combines all others in this figure.}
\end{figure}
